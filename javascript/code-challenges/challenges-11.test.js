...
const count = (target, input) => {
  return input.join().split(',').filter(num => Number(num) === target).length;
};
...
const totalSum = (input) => {
  let total = 0;
  input.forEach(arr =>
    total += arr.reduce((accum, curVal) => accum + curVal, 0));
  return total;
};
...
let findMaleAndFemale = (data) => {
  let names = [];
  data.forEach(obj => {if (obj.gender === 'male' || obj.gender === 'female'){names.push(obj.name);}});
...
let findShortest = (data) => {
  let number = data.reduce((accum, curValue)=> {
    if (accum > parseInt(curValue.height)) {
      return parseInt(curValue.height);
    } else {
      return accum;
    }
  }, 1000000);
  return data.find(obj => parseInt(obj.height) === number).name
};
