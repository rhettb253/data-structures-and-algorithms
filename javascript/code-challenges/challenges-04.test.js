...
const alphabetizeBetter = (arr) => {
  ...
  arr.forEach(word => {
    if (word.charAt(0) === word.charAt(0).toUpperCase()) {
      startedUpper.push(word.toLowerCase());
    }
  });
  let sortedArr = arr.map(val => val.toLowerCase()).sort(); //sorted, now add the caps back
  finalArr = sortedArr.map(word => {
    if (startedUpper.includes(word)) {
...
//   arr.forEach(word => {
//     if (word.charAt(0) === word.charAt(0).toUpperCase()){
//       startedUpper.push(word);
//       newArr.push(word);
//     } else {
//       startedLower.push(word);
//       newArr.push(word);
//     }
//   });
//   newArr.sort();
