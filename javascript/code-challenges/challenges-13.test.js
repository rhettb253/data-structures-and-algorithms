...
const longestString = (arr) => {
  let value = arr.reduce((accum, curVal) => curVal.length > accum.length ? curVal : accum ,'');
...
const findHappiness = (arr) => {
  return arr.filter(stringy => stringy.includes(':)'));
};

// return arr.map(stringy => stringy.includes(':)') && stringy);
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(stringy => stringy.replace(/\D/g, ''));
};
